...
Для пофайлового review запустите `python scripts/source_inventory_report.py --current data/source-inventory-current.json --reviewed --private-queue data/source-review-queue.json` из корня репозитория. Новый JSON создаётся только в реально ignored `data/` и содержит private IDs, пути, статусы registry/processing и связи с темами; существующий файл не перезаписывается. Его нельзя включать в PR или assets. При наличии `--previous` отдельное состояние `relocated` означает перемещение файла без изменения его содержательной редакции: требуется перепроверить lesson link, а не автоматически переутвердить derivative.
