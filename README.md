...
Для приватного рекурсивного metadata export Drive используйте `python scripts/source_inventory_report.py --current data/source-inventory-current.json` из корня репозитория. Опциональные `--previous`, `--processed` и `--links` принимают приватные JSON snapshots того же обхода, обработанных редакций и подтверждённых связей занятий; формат и границы публикации описаны в [content rollout](docs/question_bank_content_rollout.md#проверка-происхождения-и-публикация). Держите входные файлы в ignored `data/`: по умолчанию команда только читает их и выводит агрегаты без file IDs, названий и путей. Неполную pagination chain она отвергает.

`--reviewed` в inventory report дополнительно сверяет приватный metadata snapshot с `content/source-corpus.json` и `content/curriculum.json`: выводит только агрегаты по текущим, изменённым, перемещённым и отсутствующим источникам и связей с занятиями. Совпадение metadata не подтверждает содержательный review. Без `--processed` поле `new_unprocessed` означает отсутствие переданного private processing snapshot, а не отсутствие уже выполненного review; без `--links` поле `linked_lessons` не учитывает связи из curriculum (`reviewed_graph.reviewed_lesson_edges`).

Для пофайлового review запустите `python scripts/source_inventory_report.py --current data/source-inventory-current.json --reviewed --private-queue data/source-review-queue.json` из корня репозитория. Новый JSON создаётся только в реально ignored `data/` и содержит private IDs, пути, статусы registry/processing, связи с темами и item IDs из `learning-quality-reviews.json`; partial/disputed или устаревшее source evidence попадает в отдельный priority list без автоматического approval. Существующий файл не перезаписывается; его нельзя включать в PR или assets. При наличии `--previous` отдельное состояние `relocated` означает перемещение файла без изменения его содержательной редакции: требуется перепроверить lesson link, а не автоматически переутвердить derivative.

Для обнаруженного содержательного противоречия `python scripts/source_conflict.py --current data/source-inventory-current.json --prior data/source-processing-current.json --source-id ID --related-source-id ID --reason 'Что расходится' --locator 'Раздел или страница' --output data/source-processing-next.json` создаёт новый приватный snapshot с `conflict_review`. Он не исправляет исходный Drive-файл и не утверждает derivative. Перед последующей публикацией редактор проверяет обе версии и обновляет review; при повторном запуске inventory report передайте этот snapshot через `--processed`, чтобы явный конфликт поставил уже связанные derivatives в очередь проверки даже при неизменной Drive metadata.
Повторный `source_capture.py` не снимает конфликт той же редакции; после новой редакции Drive он возвращает источник в `pending_review` с новым digest.

После содержательного review захваченного файла `python scripts/source_finalize.py --current data/source-inventory-current.json --prior data/source-processing-current.json --source-id ID --content data/source-extract.txt --reviewer 'Имя редактора' --review-note 'Что и по каким разделам проверено' --output data/source-processing-next.json` записывает отдельное приватное решение `processed`. Команда сверяет Drive metadata revision и SHA-256 с `pending_review`, не снимает `conflict_review`, не утверждает derivative и не публикует исходный файл. `--content` должен указывать на те же сохранённые байты, что использовались при capture.
