...
- `ADMIN_TELEGRAM_IDS` — опционально: список Telegram user id администраторов через запятую (только числа), например `ADMIN_TELEGRAM_IDS=123456789,987654321`.

## Скрытая owner-only аналитика

- Команда `/stats` доступна только пользователям, чьи числовые Telegram user id указаны в `ADMIN_TELEGRAM_IDS`.
- Команда скрыта из публичного списка команд, help-текста и пользовательской клавиатуры.
