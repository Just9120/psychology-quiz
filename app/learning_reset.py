"""Explicit, actor-scoped quiz reset; the caller owns the transaction.
...
def _read(conn, actor, payload):
    ...
    available = sorted(set(topics.values()))
    ...
    state = [actor, mode, topic, *[[list(row) for row in rows] for rows in (sessions, questions, answers)]]
