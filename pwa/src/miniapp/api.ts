...
export type MiniLiteratureItem = { id: string; topic_id: string; title?: string; authors?: string[]; year?: number | null; why_read?: string; topic_order?: number | null; priority?: string | null; outbound_links?: { format: 'text' | 'audio'; access: 'free' | 'paid' | 'subscription' | 'catalog'; label: string; url: string; rights_note: string; verified_at: string }[]; source?: { title?: string; citation?: string }; user_state?: { reading_status: ReadingStatus; progress_percent: number | null } | null }
