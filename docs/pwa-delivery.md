...
После подготовки кода пользователь 19.09.2026 поручил настройку VPS и публикацию PWA. Target и config owner установлены; первичный HTTP маршрут и сертификат подготовлены оператором. Активация backend/static, local HTTPS и независимый public smoke подтверждены; VPS-side public probe завершился URLError и требует отдельной диагностики. Real-mail/owner acceptance пока PENDING; результаты и ограничения — в E-PWA-05 плана. [Backend auth/config](pwa-auth.md), [existing VPS procedure](miniapp-deployment-qa.md) и [client/build](pwa-client.md) сохраняют свои роли. Canonical frontend commands — [README](../README.md#pwa-local-run-и-проверки).
