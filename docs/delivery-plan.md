...
| AC-AUTH-01 | IN_PROGRESS | MANDATORY | PR2 default-off independent e-mail/password API и owner allowlist, negative integration PASS (E-PWA-02); PWA UI/onboarding ещё PR3. |
| AC-AUTH-02 | IN_PROGRESS | MANDATORY | One-time verify/recover, atomic consume, Argon2id и SMTP TLS/error handling tests PASS (E-PWA-02); Яндекс sender/config и real-mail delivery отложены пользователем. |
...
| AC-AUTH-05 | IN_PROGRESS | MANDATORY | PR2: bot confirmation + bound PWA complete, expiry/conflict/replay rejection и legacy preservation PASS (E-PWA-02); user-facing PWA linking screen ещё PR3. |
...
| AC-OPS-05 | IN_PROGRESS | MANDATORY | Auth/access и privacy/destination negative tests PASS; F-021/028 закрыты #286/E-STAB-03. Owner web security tests и F-023 candidate fix — E-PWA-02; полный platform/security scope остаётся. |
...
| F-023 / CLOSED (PR2 candidate) | Известные advisories в resolved requirements. | Audit E-DEPS: 8 advisory IDs в python-dotenv 1.0.1/Starlette 0.46.2, FastAPI pin ограничивал upgrade. PR2 E-PWA-02: fixed compatible pins, full behavior suite PASS и fresh pip-audit — 0 known vulnerabilities. P2; HIGH для dependency snapshot. | FIX выполнен в PR2 worktree; production rollout pending CI/merge/CD. Reachability прежних form/FileResponse/set_key paths не обнаружена, но новые auth inputs требуют fixed versions. Transitive lock/base images остаются F-015; security completeness не заявлена. |
...
19.09.2026: PWA-FIRST-001 ACTIVE. PR1 #290 merged/delivered `ba7f627`, CI/CD и preservation PASS (PR2 policy / E-PWA-01); main синхронизирован, ветки PR1 удалены. PR2 `codex/pwa-owner-auth` от `ba7f627ccbbccd5243ebbc0c2b8b5cdb28045d22`: auth-v1, owner mail/password/session/linking, adapter routes/tests и related dependency fix реализованы, local checks E-PWA-02 PASS; local smoke/self-review завершены; осталось publish/checks/merge/CD. Policies/Validation выше закреплены до кода. Domain/mail/owner settings по просьбе пользователя позже; новые routes default-off, secrets/DNS не меняются.

После local validation/self-review — PR/checks/merge и existing safe backend delivery с verified backup/new auth-table preservation/exact revision. Затем frontend PR на свежем main. PWA publication/real-mail acceptance остаются отложенными; это не code READY/Goal DONE до подтверждения соответствующего этапа. Процент проекта не пересчитывался.
