...
- LEARN-CONTOURS-GLOSSARY-MVP-001 completed: expanded the first static qualitative-methods glossary MVP to 8 approved supplied-snippet entries and added `python scripts/validate_glossary.py`; scope remains content/validation-only with no Telegram UI, runtime loading, database, seed, CI/CD, deploy, or source-certification changes. Follow-up: expand only from supplied or available source excerpts and wire CI in a separate focused task if desired.
