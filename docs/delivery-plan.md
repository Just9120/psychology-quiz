...
| 2. API input/privacy/destination | F-020/021/028 закрыты regression tests; 4xx без state mutation, отсутствие raw user IDs/initData в logs и невозможность отправки auth на arbitrary origin; совместимые clients | IN_PROGRESS: codex/stabilization-api-boundaries, base 8c53aba |
...
PLATFORM-STABILIZATION-001 активна; PWA остаётся следующим приоритетом. Foundation поставлен через #284 (`9d6d6cc`) и содержательный SSH transport hotfix #285 (`8c53ababe5796794df790aac9bc95c5f15a89750`). CI 35447211955 PASS; CD 35447236343/job 105908164003 PASS 19.09 13:57 UTC: exact source/image identity, verified backup/isolated restore, user preservation, DB smoke, internal HTTP version/auth checks. `stateful=1 migrate=0`: первая versioned adoption не переписывала существующий content. Первый CD 35446837711 дошёл только до preflight и не считается delivery evidence. Обе собственные merged branches удалены после проверки squash tree equivalence; чужих изменений нет.

Текущий PR2: `codex/stabilization-api-boundaries`, base `8c53ababe5796794df790aac9bc95c5f15a89750`, fresh main/settings проверены. Scope F-020/021/028, Validation Plan выше; strict type/enum/category validation до DB writes, privacy-safe telemetry и published trusted API config независимо от URL context. Existing Telegram entrypoint/context fallback сохраняются; новая auth/account/PWA архитектура не вводится. Проверить actual fetch behavior, synthetic malformed requests/no state mutation, оба API adapters и affected legacy handlers; backend + static frontend delivery обязательны.

PR2 local: 184 affected tests/16 subtests PASS; после удаления unused credentials в logger 123 runner/API tests/8 subtests PASS. Node behavior запрещает arbitrary origin/userinfo/port/scheme/path и redirects до fetch; IAB synthetic valid legacy context с hostile api_base_url показывает trusted origin и рабочую форму, console warning/error нет. Authenticated mobile browser N/A для этой проверки; synthetic authorization проверена API/Node tests. Итоговые all/mix category, legacy HTTP privacy, config/logging/glossary/literature: 86 tests/8 subtests PASS; compileall/diff/relative links PASS. Self-review input/auth/logging/fetch boundaries выполнен. Следующий шаг: initial push/PR, CI/merge/backend+static delivery; до них delivery PENDING.

После PR2 — PR3 glossary retry и PR4 history/lifecycle. Новый процент готовности не вычислять. Goal закрывается только после всего scope/обязательных checks/merge/поставки, затем остановиться для выбора PWA Goal.
