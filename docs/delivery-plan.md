...
## Near backlog
- MINIAPP-CONTOUR-ENTRYPOINT-RESTORE-001 completed: restored `/ui` and `🚀 В окне` as Mini App setup/contour chooser entrypoints even when a normal quiz runner is active; active-session launches keep the new-quiz warning, setup context keeps glossary metadata, and post-setup runner/result reopen links retain runner/completed behavior.
- INFRA-CD-MINIAPP-API-DEPLOY-001 completed: production CD/deploy now rebuilds and recreates both Compose runtime services (`psych_quiz_bot` and `psych_quiz_miniapp_api`) on app runtime restarts, with a post-deploy `docker compose ps` check and Mini App API log tail; DB init/seed behavior remains limited to the existing deploy seed path and secrets behavior is unchanged.
- LEARN-CONTOURS-GLOSSARY-MINIAPP-START-HOTFIX-001 completed: fixed Mini App glossary start after topic/count selection by normalizing glossary question_count payloads and adding controlled start-error handling; topic picker still opens from launch context, quiz start/answer/next/retry continue through existing Mini App endpoints; normal Mini App quiz remains unchanged; no DB/seed/literature/deploy behavior changes.
- LEARN-CONTOURS-GLOSSARY-MINIAPP-CONTEXT-HOTFIX-001 completed: embedded safe glossary topic metadata into Mini App launch context so the Glossary mode can open its topic picker without relying on a runtime setup-options request; glossary quiz start/answer/next/retry still use existing Mini App endpoints; normal Mini App quiz remains unchanged; no DB/seed/literature/deploy behavior changes.
- LEARN-CONTOURS-GLOSSARY-MINIAPP-EXISTING-ENDPOINTS-HOTFIX-001 completed: fixed Mini App glossary production routing by moving the user-facing glossary flow onto existing Mini App endpoints (/miniapp/setup-options, /miniapp/setup, /miniapp/answer), removing frontend dependency on /miniapp/glossary/* routes for open/start/answer/next/retry; normal Mini App topic quiz remains unchanged; source_refs remain internal and hidden; no DB/seed/literature/deploy changes.
- LEARN-CONTOURS-GLOSSARY-MINIAPP-SETUP-FALLBACK-HOTFIX-001 completed: made Mini App glossary opening prefer cached/setup-options glossary topics before using `/miniapp/glossary/topics`, added backward-compatible setup-options glossary aliases, and kept controlled user-facing errors only after all glossary topic sources fail; normal Mini App quiz remains unchanged; no DB/seed/literature/deploy behavior changes.
