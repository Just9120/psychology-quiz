...
| 3. Glossary retry | F-019: stable step/options, answer/next idempotency и concurrent/retry tests; dedicated/legacy routes и chat совместимы; old no-step requests требуют refresh, без нового durable learning subsystem | IN_PROGRESS: codex/stabilization-glossary-retries, base 9346ab1 |
...
PLATFORM-STABILIZATION-001 активна, PWA — следующий приоритет. Foundation #284/#285 поставлен на 8c53aba (E-STAB-02). API/privacy/destination #286 поставлен на `9346ab1b2ab6df43d6930f1b8dd195077cf5a1f1`: CI 35448069825, CD 35448090503/job 105910416453 PASS 14:14 UTC; `stateful=0 migrate=0`, DB/HTTP version smoke и оба image labels PASS. Cloudflare main build 0bbf49fb-b44b-4c9f-b91b-939da51b10df PASS; опубликованные index.html/api-config.js по нормализованному text полностью совпали с main (HTTP 200). Ветка #286 удалена после squash tree check; user changes нет.

Текущий PR3: `codex/stabilization-glossary-retries`, base `9346ab1b2ab6df43d6930f1b8dd195077cf5a1f1`, fresh main/GitHub/worktree проверены. До реализации закреплён step_id contract в spec: cached question/feedback/next, session lock, owner isolation; UI передаёт step_id и обрабатывает lost response/retry, старый no-step формат безопасно отклоняется с refresh-required. Иначе невозможно отличить задержанный ответ от нового. Dedicated/existing endpoint routes, chat glossary и normal quiz сохраняются. Новая frontend версия может работать со старым backend через existing order_index во время rollout; новые server guards запрещают ambiguous cached old-client writes.

Validation PR3: duplicate/concurrent answer/next/restart, stable options/score, stale-step/wrong-owner/invalid selection, no mutation при missing token; API contracts и UI request/retry behavior. F-018 durable persistence/retention не реализуется. Local validation и self-review завершены (E-STAB-04); остаются CI/merge/backend+static delivery актуальной revision, успех пока не предполагается. Затем только PR4 history/publication; readiness snapshot не пересчитывать, Goal пока не завершена.
