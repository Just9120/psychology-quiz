...
В D-15 glossary attempts, snapshots, ответы и результат хранятся в общей БД actor; PWA, Mini App и classic `/glossary` восстанавливают текущий вопрос/feedback либо последний завершённый результат. Замена активной попытки требует явного подтверждения и её актуального ID; чужая, сброшенная или заменённая попытка не изменяется поздним запросом. Источник может измениться или стать недоступным без изменения уже показанных вариантов/definitions. Общий reset включает glossary: по теме — по сохранённому названию темы попытки, целиком — все glossary attempts только выбранного actor. Quiz history/aggregates пока не смешиваются с glossary результатами; due-list/knowledge links остаются E04/E06. До перехода прежние in-memory попытки не имеют durable исходника для переноса; сохранённые quiz/literature данные переносятся без потери. Проверка: обе БД, новый процесс, concurrent replay, API auth/CSRF, межклиентное восстановление и reset rollback.
